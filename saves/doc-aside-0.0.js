class docAside extends HTMLElement {
  ...
  connectedCallback() {
    ...
    <style>
      ...
      doc-aside {
        ...
        padding: var(--doc-aside-padding) 0;
        position: fixed !important;
        top: var(--nav-height, 1rem);
        left: 0;
        bottom: 0;
      ...
      doc-aside .doc-aside {
        ...
        // ol li summary { background-color: hsla(var(--o-bg), .2)}
        ...
        :root h1, :root h2, :root h3, :root h4, :root h5, :root h6 {
          scroll-margin-top: calc(var(--nav-height) + var(--space-unit) * 3) !important;
        }
      }
    </style>
  ...
  structureAside() {
    ...
    const buildNavigation = (docBodyElement) => {
      const headings = docBodyElement.querySelectorAll('h1, h2, h3, h4, h5, h6');
    ...
    this.buildHierarchicalNav = (headings, container) => {
      ...
      const processHeadings = (parentContainer, maxLevel = 6) => {
        while (i < headingArray.length) {
          ...
          li.style.marginLeft = `calc(var(--space-unit) * 2 * ${level - 1})`;

          if (hasChildren) {
            // Create details/summary structure
            const details = document.createElement('details');
            
            // Open level 1 (h1) elements by default
            if (level === 1) {
              details.open = true;
            }
