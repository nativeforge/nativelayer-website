 ...
 * ::
 * 
 * Attribute-only Blocks (apply attributes to markdown elements):
...
class remd extends HTMLElement {
      ...
      re-md {
        ...
        * {
        ...
        :where( ul, ol ), :where(ul ol) {
          padding: calc(var(--space-unit, 0.5rem) * 1) 0 calc(var(--space-unit, 0.5rem) * 1) calc(var(--space-unit, 0.5rem) * 2);
        }

        hr {
          padding: calc(var(--space-unit, 0.5rem) * 2) 0;
          border: none;
          border-top: 1px dotted hsla(from var(--color) h s l / 0.5);
        }

        blockquote {
          margin: 0;
          padding: calc(var(--space-unit, 0.5rem) * 2) calc(var(--space-unit, 0.5rem) * 4);
          background-color: hsla(from var(--color) h s l / 0.07);
          border-radius: calc(var(--size-unit, 0.5rem) * 0.25);
          border-left: 4px solid hsla(from var(--color) h s l / 0.4);
        }

        code {
          font-size: var(--f-size-p, 1rem);
          padding: calc(var(--space-unit, 0.5rem) * 1/3) var(--space-unit, 0.5rem);
          background-color: hsla(from var(--color) h s l / 0.06);
  ...
  async processComponentWithSlots(content, componentName, componentAttributes, transformRules) {
    ...
    // Special case: No component name specified - apply attributes to markdown content
    if (!componentName) {
      const processedContent = await this.markdownToHtml(content, transformRules);
      
      // If no attributes, just return the processed content
      if (!attrs) {
        return processedContent;
      }
      
      // Inject attributes into the first element
      return this.injectAttributesIntoFirstElement(processedContent, componentAttributes);
    }
  ...
  async renderCodeBlock(code, language, attributes = {}) {
    ...
    const componentCSS = `
      ...
      .code-block-${uniqueId} .wrapper {
        ...
        line-height: 1.42;
        background-color: hsla(var(--o-bg, ${bgHslAttr}), .07);
